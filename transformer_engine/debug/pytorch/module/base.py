...
try:
    ...
    raise ModuleNotFoundError("ERROR: Could not locate debug_tool package. Make sure it is installed correctly.")
...
class TransformerEngineBaseModule(torch.nn.Module, ABC):
    ...
    def set_fp8_weights(self) -> None:
        ...
        for i, shape in enumerate(self.fp8_weight_shapes, start=1):
            ...
            weight_cs_cast_attr = f"weight{i}_fp8_cs"
    ...
    def init_fp8_metadata(self, num_gemms: int = 1) -> None:
        ...
        if self.fp8_parameters: 
            debug_api.log_message("> Primary FP8 parameters is not supported in the debug module. "
                                "Using this flag will not affect the debug module. ", 
                                level=logging.WARNING)
            
        self.fp8 = debug_api.transformer_engine.is_fp8_enabled_for_layer(self.name, fp8_enabled=FP8GlobalStateManager.is_fp8_enabled())["ret"]
        self.fp8_calibration = FP8GlobalStateManager.is_fp8_calibration()
        if self.fp8_calibration:
            debug_api.log_message("> FP8 calibration is not supported in the debug module. "
                                "Using this flag will not affect the debug module. ", 
                                level=logging.WARNING)
        ...
        if self.fp8:
            ...
            if self.fp8_meta["recipe"].override_linear_precision.wgrad:
                debug_api.log_message("> Disabling FP8 Wgrad through --fp8-wgrad argument is not supported in debug mode. "
                                    "Use debug config. ", 
                                    level=logging.WARNING)
