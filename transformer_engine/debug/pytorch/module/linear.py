...
def maybe_gather_tensor(tensor, parallel_mode, sequence_parallel, tp_group):
    if parallel_mode == "column" and sequence_parallel:
        tensor_total, handle = gather_along_first_dim(tensor, tp_group)
...
class _Linear(torch.autograd.Function):
    ...
    def forward(
        ...
        weight_fp8_ds: Union[Float8Tensor, None],
        weight_fp8_cs: Union[Float8Tensor, None],
        inp: torch.Tensor,
        bias: torch.Tensor,
        use_bias: bool,
        ...
        if fp8:
            ...
            if DebugLayerState.get(name).DelayedScaling.FPROP or DebugLayerState.get(name).DelayedScaling.WGRAD:
            ...
            if update_fp8_weights:
                if DebugLayerState.get(name).DelayedScaling.FPROP or DebugLayerState.get(name).DelayedScaling.DGRAD:
                    weight_fp8_ds = Float8Tensor(
                        data=weight_fp8_ds._data,
                        fp8_meta=fp8_meta,
        ...
        # generic stats
        debug_api.collect_tensor_stats(name, tensor=inputmat_no_fp8_total, tensor_name="activation", iteration=step_counter)
        debug_api.collect_tensor_stats(name, tensor=weight, tensor_name="weight", iteration=step_counter)
        
        # transformer engine speciifc stats
        debug_api.transformer_engine.collect_tensor_stats(name, tensor=inputmat_no_fp8_total, tensor_name="activation", iteration=step_counter)
        debug_api.transformer_engine.collect_tensor_stats(name, tensor=weight, tensor_name="weight", iteration=step_counter)

        if DebugLayerState.get(name).FP8Gemm.FPROP:
            # Activation cast
            qinputmat_dict = {}
            if DebugLayerState.get(name).DelayedScaling.FPROP:
                debug_api.log_message("Fprop: Delayed Scaling", name)
                ...
                debug_api.log_message("Fprop: Current Scaling", name)
                qinputmat_dict.update(debug_api.transformer_engine.process_tensor(name, gemm="fprop", tensor=inputmat_no_fp8_total, fp8_enabled=fp8, fp8_dtype=fp8_dtype_forward))
                qinputmat_dict["index"] = 0

            # Weight cast
            qweight_dict = {}
            if update_fp8_weights:
                if not DebugLayerState.get(name).DelayedScaling.FPROP:
                    weight_fp8_cs = Float8Tensor(
                        data=weight_fp8_cs._data,
                        fp8_meta=fp8_meta,
                        fp8_meta_index=0,
                    )
                    ret = debug_api.transformer_engine.process_tensor(name, gemm="fprop", tensor=weight, out=weight_fp8_cs._data, fp8_enabled=fp8, fp8_dtype=fp8_dtype_forward)
                    DebugLayerState.set_current_scale_inv(name, ret["scale_inv"])

            if DebugLayerState.get(name).DelayedScaling.FPROP:
                qweight_dict["tensor"] = weight_fp8_ds._data
                qweight_dict["scale_inv"] = fp8_meta["scaling_fwd"].scale_inv
                qweight_dict["index"] = tex.FP8FwdTensors.GEMM1_WEIGHT
            else:
                qweight_dict["tensor"] = weight_fp8_cs._data
                qweight_dict["scale_inv"] = DebugLayerState.get_current_scale_inv(name)
                qweight_dict["index"] = 0

            debug_api.transformer_engine.collect_fp8_tensor_stats(name, tensor=qinputmat_dict["tensor"], tensor_name="activation", iteration=step_counter)
            debug_api.transformer_engine.collect_fp8_tensor_stats(name, tensor=qweight_dict["tensor"], tensor_name="weight", iteration=step_counter)
            ...
            debug_api.log_message("Fprop: BF16", name)
            # Cast for native AMP
            weight = cast_if_needed(weight, activation_dtype)
            bias = cast_if_needed(bias, activation_dtype) if use_bias else bias

            inputmat_dict = debug_api.transformer_engine.process_tensor(name, gemm="fprop", tensor_name="activation", tensor=inputmat, fp8_enabled=fp8)
            weight_dict = debug_api.transformer_engine.process_tensor(name, gemm="fprop", tensor_name="weight", tensor=weight, fp8_enabled=fp8)
        ...
        if is_grad_enabled:
            ...
            if fp8:
                if weight_requires_grad and DebugLayerState.get(name).DelayedScaling.WGRAD:
                    saved_fp8_inputmat = inputmat_fp8_ds
                    saved_inputmat = None
            
                if DebugLayerState.get(name).DelayedScaling.DGRAD:
                    saved_fp8_weight = weight_fp8_ds._data
                    saved_weight = None
    ...
    def backward(
        ...
        with torch.cuda.nvtx.range("_Linear_backward"):
            ...
            if ctx.fp8:
                ...
                if DebugLayerState.get(ctx.name).DelayedScaling.DGRAD or DebugLayerState.get(ctx.name).DelayedScaling.WGRAD:
                    grad_output_c = cast_to_fp8(
                            grad_output,
                            ctx.fp8_meta["scaling_bwd"],
                            tex.FP8BwdTensors.GRAD_OUTPUT1,
                            fp8_dtype_backward)
                    grad_output_c, _ = maybe_gather_tensor(grad_output_c, ctx.parallel_mode, ctx.sequence_parallel, ctx.tp_group)
            
            grad_output, _ = maybe_gather_tensor(grad_output, ctx.parallel_mode, ctx.sequence_parallel, ctx.tp_group)
            debug_api.collect_tensor_stats(ctx.name, tensor=grad_output, tensor_name="gradient", iteration=ctx.step_counter)
            debug_api.transformer_engine.collect_tensor_stats(ctx.name, tensor=grad_output, tensor_name="gradient", iteration=ctx.step_counter)

            if DebugLayerState.get(ctx.name).DelayedScaling.WGRAD:
                inputmat_total, handle = maybe_gather_tensor(inputmat_fp8, ctx.parallel_mode, ctx.sequence_parallel, ctx.tp_group)
            else:
                inputmat_total, handle = maybe_gather_tensor(inputmat, ctx.parallel_mode, ctx.sequence_parallel, ctx.tp_group)
            ...
            if ctx.requires_dgrad:
                if DebugLayerState.get(ctx.name).FP8Gemm.DGRAD:
                    ...
                    if DebugLayerState.get(ctx.name).DelayedScaling.DGRAD:
                        debug_api.log_message("Dgrad: Delayed Scaling", ctx.name)
                        ...
                        assert  weight_fp8 != None
                        dgrad_qweight_dict["tensor"] = tex.fp8_transpose(weight_fp8, fp8_dtype_forward)
                        dgrad_qweight_dict["scale_inv"] = fwd_scale_inverses
                        dgrad_qweight_dict["index"] = tex.FP8FwdTensors.GEMM1_WEIGHT
                    else:
                        debug_api.log_message("Dgrad: Current Scaling", ctx.name)
                        dgrad_qgrad_dict = debug_api.transformer_engine.process_gradient_tensor(ctx.name, gemm="dgrad", tensor=grad_output, fp8_enabled=ctx.fp8, fp8_dtype=fp8_dtype_backward)
                        dgrad_qgrad_dict["index"] = 0
                        dgrad_qweight_dict = debug_api.transformer_engine.process_tensor(ctx.name, gemm="dgrad", tensor=weight.t().contiguous(), fp8_enabled=ctx.fp8, fp8_dtype=fp8_dtype_forward)
                        dgrad_qweight_dict["index"] = 0
                    
                    debug_api.transformer_engine.collect_fp8_tensor_stats(ctx.name, tensor=dgrad_qgrad_dict["tensor"], tensor_name="gradient", iteration=ctx.step_counter)
                    ...
                    debug_api.log_message("Dgrad: BF16", ctx.name)
                    dgrad_grad_dict = debug_api.transformer_engine.process_gradient_tensor(ctx.name, tensor=grad_output, tensor_name="gradient",
                                                                         gemm="dgrad", fp8_enabled=ctx.fp8)
                    dgrad_weight_dict = debug_api.transformer_engine.process_tensor(ctx.name, tensor=weight, tensor_name="weight",
                                                                gemm="dgrad", fp8_enabled=ctx.fp8)
                    dgrad, _, _ = gemm(
            ...
            if ctx.weight_requires_grad:
                if DebugLayerState.get(ctx.name).FP8Gemm.WGRAD:
                    ...
                    if DebugLayerState.get(ctx.name).DelayedScaling.WGRAD:
                        debug_api.log_message("Wgrad: Delayed Scaling", ctx.name)
                        ...
                        debug_api.log_message("Wgrad: Current Scaling", ctx.name)
                        wgrad_qgrad_dict = debug_api.transformer_engine.process_gradient_tensor(ctx.name, gemm="wgrad", tensor=grad_output.t().contiguous(), fp8_enabled=ctx.fp8, fp8_dtype=fp8_dtype_backward)
                        wgrad_qgrad_dict["index"] = 0
                        wgrad_qinput_dict = debug_api.transformer_engine.process_tensor(ctx.name, gemm="wgrad", tensor=inputmat_total.t().contiguous(), fp8_enabled=ctx.fp8, fp8_dtype=fp8_dtype_forward)
                    ...
                    debug_api.log_message("Wgrad: BF16", ctx.name)
                    wgrad_input_dict = debug_api.transformer_engine.process_tensor(ctx.name, tensor=inputmat_total, tensor_name="activation",
                                                                    gemm="wgrad", fp8_enabled=ctx.fp8)
                    wgrad_grad_dict = debug_api.transformer_engine.process_gradient_tensor(ctx.name, tensor=grad_output, tensor_name="gradient",
                                                                         gemm="wgrad", fp8_enabled=ctx.fp8)
        ...
        debug_api.stop_layer_logging(ctx.name)
        return (
            wgrad,
            None,
            None,
...
class Linear(TransformerEngineBaseModule):
    ...
    def __init__(
        ...
        if self.name == None:
            debug_api.log_message("[DEBUG-WARNING] Names are not provided to debug modules. ",
    ...
    def get_fp8_weights_scratchpad(
        ...
        if is_first_microbatch is None:
            ...
            fp8_weight_tensors = [self.weight1_fp8, self.weight1_fp8_cs]
